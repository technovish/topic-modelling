...
def identify_topic_gemini(comment):
    ...
    try:
        ...
        prompt_updated = f"""Task: Read the following customer feedback carefully and identify the single most relevant category for it.
Instructions:
- Choose only ONE category from the list below
- If the comment doesn't clearly fit any category, choose 'Others'
- Respond with ONLY the category name, nothing else

Categories:
Product Quality - Feedback about the product itself (features, performance, bugs, design, etc.)
Service Quality - Feedback about customer service, shipping, delivery, website experience, etc.
Agent - Specific mentions of interactions with staff or representatives
Price - Comments about cost, value, pricing, or discounts
Bills/Payment - Specific mentions of bills or payments or Feedback about bills or payments or charges.
Issue Resolution - Specific mentions of issue resolution or feedback about issue resolution.
Others - Doesn't fit any of the above or is unclear
...
def analyze_file(file_path):
    ...
    def process_comment(item):
        i, comment = item
        # Retry mechanism in case we hit rate limits with parallel requests
        for attempt in range(3):
            topic = identify_topic_gemini(comment)
            if topic != "Service Issue" or attempt == 2:
                return i, topic
            time.sleep(1 + attempt * 2) # Backoff before retry
        return i, "Service Issue"

    # Process comments in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_comment, item) for item in comments.items()]
        completed = 0
        for future in concurrent.futures.as_completed(futures):
            i, topic = future.result()
            data.at[i, 'identified_topic'] = topic
            completed += 1
            if completed % 10 == 0:
                print(f"Processed {completed} comments for topic identification.")
